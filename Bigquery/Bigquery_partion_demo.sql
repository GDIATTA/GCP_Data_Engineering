/************** BigQuery_partition_demo  *******************/
/************** firstime : Create a dataset "bigquery_demos" *******************/
/************** secondtime : Copy table "bikeshare_trips from dataset "austin_bikeshare" in project "bigquery-public-data" into a dataset "bigquery_demos" *******************/


SELECT * FROM `fine-arbor-421713.bigquery_demos.bikeshare_trips` LIMIT 1000;

/************** Time Unit Partitioning  *******************/

-- Query this table to understand the data distribution across different dates

SELECT  min(start_time), max(start_time) FROM `fine-arbor-421713.bigquery_demos.bikeshare_trips`;

select DATE_TRUNC(start_time, DAY) as year,count(*) from `fine-arbor-421713.bigquery_demos.bikeshare_trips`
group by 1 order by 1;

select DATE_TRUNC(start_time, MONTH) as year,count(*) from `fine-arbor-421713.bigquery_demos.bikeshare_trips`
group by 1 order by 1;
...
PARTITION BY
  ...
  AS (SELECT  TIMESTAMP_TRUNC(start_time , DAY)
      FROM `fine-arbor-421713.bigquery_demos.bikeshare_trips`);
